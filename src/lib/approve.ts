 ...
 *    덮으면 앞 도구가 영영 안 끝난다. */
export function askApprove(o: { title: string; body?: string; hard?: boolean }): Promise<boolean> {
  if (useLlm.getState().confirm) return Promise.resolve(false);
  // ★패널을 접어 두었거나 창을 안 보고 있으면 못 본다 — 창 제목으로 알린다
