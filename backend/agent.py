...
class Tools:
    ...
    def _table(self) -> list[tuple[str, str, dict, Callable]]:
        ...
        return [
            ...
            (
                "generate",
                "★**생성을 큐에 넣는다.** 싱글 탭이면 count 장, 포즈세트 탭이면 잠기지 않은 슬롯 "
                "전부를 count 바퀴 돈다. 비우면 지금 보고 있는 탭, workspace·tab 을 주면 **거기로** "
                "넣는다 (화면은 안 옮긴다). 앱이 켜져 있어야 한다. Anlas 가 든다 — 사용자가 장수를 "
                "말했을 때만 쓴다.",
                obj({"count": n("몇 장(싱글) 또는 몇 바퀴(포즈세트). 기본 1"),
