...
씬을 줄에 늘어놓고 한 번 누르면 한 벌이 통째로 나옵니다.
...
한 장씩 뽑는 것은 어렵지 않습니다. 같은 인물을 장면만 바꿔 스무 장 뽑으려 할 때부터
일이 되지요. PeroPix 는 그 자리를 위한 도구입니다 — **그대로 두는 것은 한 번만 적고,
달라지는 것만 줄에 늘어놓은 뒤 함께 돌립니다.**

- **씬 줄** — 씬 하나가 카드 하나입니다. 다른 부분만 채우고 줄째로 돌리면, 결과가 각자
  자기를 만든 씬에 그대로 붙어 남습니다.
...
- **인페인트와 강화** — 마스크를 칠해 그 자리만 다시 그리거나, 완성된 그림을 세기만 바꿔
  다시 돌립니다. 바이브 전이와 업스케일도 같은 자리에 있습니다.
